...
const error = ({ msg, code, err, action, data, id }, isVerbose) => {
  if (err && err.id) {
    code = err.id;
  }
  if (code || (err && errorWithCode[err.code])) {
    console.log(
      errorTemplate + `${action} | ` + errorWithCode[code || err.code]
    );
    logPrettyData(data);
    return;
  }
  if (err && (err.message || err.stack)) {
    console.log(errorTemplate + `${action} | ` + err.message || err.stack);
    logPrettyData(data);
    return;
  }

  console.log(errorTemplate + `${action} | ` + msg);
  logPrettyData(data);
  return;
};
